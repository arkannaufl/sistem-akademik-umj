...
// Daftar pelajaran contoh (bisa diganti sesuai kebutuhan)
const pelajaranSesi1 = [
  'Biologi', 'Matematika', 'Kimia', 'Fisika', 'Bahasa', 'Ekonomi', 'Sosiologi',
  'Sejarah', 'Geografi', 'Seni', 'Olahraga', 'TIK', 'Agama', 'PKN', 'IPS',
  'IPA', 'Statistika', 'Logika', 'Antropologi', 'Astronomi', 'Farmasi',
  'Teknik', 'Kedokteran', 'Psikologi', 'Akuntansi', 'Manajemen', 'Hukum',
  'Desain', 'Arsitektur', 'Pertanian', 'Peternakan'
];
const pelajaranSesi2 = [
  'Matematika', 'Biologi', 'Fisika', 'Kimia', 'Ekonomi', 'Bahasa', 'Sejarah',
  'Sosiologi', 'Seni', 'Geografi', 'TIK', 'Olahraga', 'PKN', 'Agama', 'IPA',
  'IPS', 'Logika', 'Statistika', 'Astronomi', 'Antropologi', 'Teknik',
  'Farmasi', 'Psikologi', 'Kedokteran', 'Manajemen', 'Akuntansi', 'Desain',
  'Hukum', 'Pertanian', 'Arsitektur', 'Peternakan'
];
// Isi kegiatanData[0] (Semester I) untuk 31 hari x 2 sesi
for (let h = 0; h < 31; h++) {
...
export default function PetaBlok() {
  ...
  const hariPage = hariList.slice(page * pageSize, (page + 1) * pageSize);
  ...
  const filteredHariList = search.trim()
    ? hariList.filter(h => h.hari.toLowerCase().includes(search.toLowerCase()) || h.tanggal.includes(search))
    : hariList;
  const filteredTotalPages = Math.ceil(filteredHariList.length / pageSize);
  const filteredHariPage = filteredHariList.slice(page * pageSize, (page + 1) * pageSize);
  ...
  function exportJadwalToExcel() {
    ...
    semesterKolom.forEach((_, i) => {
      merges.push({ s: { r: 0, c: 2 + i }, e: { r: 0, c: 2 + i } });
      merges.push({ s: { r: 1, c: 2 + i }, e: { r: 1, c: 2 + i } });
      merges.push({ s: { r: 2, c: 2 + i }, e: { r: 2, c: 2 + i } });
    });
    let rowIdx = 3;
    for (let h = 0; h < hariList.length; h++) {
      for (let s = 0; s < sesiHarian.length; s++) {
        if (s === 0) {
          ...
          semesterKolom.forEach((sem, semIdx) => {
    ...
    for (let h = 0; h < hariList.length; h++) {
      for (let s = 0; s < sesiHarian.length; s++) {
        ...
        if (isIstirahat) {
          ...
          semesterKolom.forEach((sem, semIdx) => {
            const val = kegiatanData[semIdx][h][s];
            if (!shouldRenderCell(semIdx, h, s)) return;
            if (val && val.durasi > 1) {
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2 mb-2">
        ...
        <div className="flex flex-wrap items-center gap-2 justify-end w-full md:w-auto">
          ...
          <span className="text-xs text-gray-500 dark:text-gray-400">
            Menampilkan {filteredHariPage.length} dari {filteredHariList.length} hari
          </span>
          <button
            onClick={prevPage}
            disabled={page === 0}
        ...
        <div className="max-w-full overflow-x-auto hide-scroll" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          ...
          <tbody>
              {filteredHariPage.length === 0 ? (
                ...
                filteredHariPage.map((hari, hariIdx) => (
              sesiHarian.map((sesi, sesiIdx) => {
                    const globalHariIdx = filteredHariList.indexOf(hari);
                const isFirstSesi = sesiIdx === 0;
                const isIstirahat = sesi.isIstirahat;
                const isEven = sesiIdx % 2 === 1;
                    const isFirstRowOfDay = isFirstSesi && globalHariIdx !== 0 && hariIdx !== 0;
                return (
                  <React.Fragment key={hari.hari + '-' + sesiIdx}>
                    {isFirstRowOfDay && (
                      <tr>
                        <td colSpan={2 + semesterKolom.length} className="h-8 bg-gray-200 dark:bg-gray-700 p-0" />
                      </tr>
                    )}
                    <tr className={getRowClass(!!isIstirahat, sesiIdx)}>
                      {isFirstSesi && (
                        <td rowSpan={sesiHarian.length} className="px-4 py-2 border-b border-gray-200 dark:border-gray-700 text-center align-middle bg-gray-50 dark:bg-gray-900 text-xs font-medium dark:text-gray-200">
