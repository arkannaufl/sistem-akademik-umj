...
export default function NotificationDropdown() {
  ...
  const loadNotifications = async (role: string, userId: number) => {
    try {
      setLoading(true);
      let response;
      ...
      try {
        ...
        if (unreadNotifications.length > 0) {
          if (user.role === 'super_admin' || user.role === 'tim_akademik') {
            ...
            // Mark all dosen notifications as read
            await Promise.all(
              unreadNotifications.map(n => api.put(`/notifications/${n.id}/read`))
            );
          }
            ...
            {loading ? (
              ...
              notifications.map((notification) => (
                <li key={notification.id}>
                  <div className={`flex gap-3 rounded-lg border-b border-gray-100 p-3 px-4.5 py-3 hover:bg-gray-100 dark:border-gray-800 dark:hover:bg-white/5 cursor-pointer ${!notification.is_read ? 'bg-blue-50 dark:bg-blue-900/20' : ''}`}>
